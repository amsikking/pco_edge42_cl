...
class Camera:
    '''
    Basic device adaptor for PCO.edge 4.2 CamLink sCMOS camera. Many more
    ...
    def __init__(self,
        ...
        try:
            ...
            print("%s: - is 'sc2_cl_me4.dll' in the 'SC2_Cam.dll'"%self.name,
                  "directory? (CamLink)")
            raise
        wSZCameraNameLen = 40
...
def legalize_image_size(
    height_px='max', width_px='max', name='PCO.edge4.2', verbose=True):
    """returns a nearby legal image size at the *center* of the camera chip"""
    height_step, width_step = 1, 20
    min_height, min_width, max_height, max_width = 10, 40, 2048, 2060
    ...
    if num_width_steps  % 2 == 0: num_width_steps  += 1 # must be odd for chip 
